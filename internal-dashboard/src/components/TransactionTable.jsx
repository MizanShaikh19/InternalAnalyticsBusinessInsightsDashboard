...
export default function TransactionTable({
    ...
    return (
        <div className="card" style={{ marginTop: 24 }}>
            ...
            <div className="table-container" style={{ padding: '0 20px 20px 20px', overflowX: 'auto' }}>
                <table className="premium-table">
                    <thead>
                        <tr>
                            <th onClick={() => handleSort('record_date')} style={{ cursor: 'pointer' }}>
                        ...
                        {(records || []).length === 0 ? (
                            ...
                            records.map((r) => (
                                <tr key={r.id}>
                                    ...
                                    <td>
                                        <span style={{ padding: '4px 10px', borderRadius: 99, background: 'rgba(0, 102, 255, 0.1)', color: 'var(--primary)', fontSize: 11, fontWeight: 800, textTransform: 'uppercase' }}>{r.category}</span>
                                    </td>
                                    <td style={{ textAlign: 'right', fontWeight: 700, fontFamily: 'monospace' }}>{formatCurrency(r.amount)}</td>
                                    <td style={{ fontSize: 11, color: 'var(--text-muted)', fontFamily: 'monospace' }}>{r.id.toUpperCase().slice(0, 12)}</td>
                                </tr>
                            ))
                        )}
                    </tbody>
